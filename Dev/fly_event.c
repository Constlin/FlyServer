...
int fly_event_add(fly_event *ev, const struct timeval *tv) 
{
    if (ev == NULL) {
    	return -1;
    }
    if (tv == NULL) {
        ...
        //tv != NULL, meaning it's time event
        if (ev->status & FLY_LIST_REG) {
            //Todo: code a minheap to save the time event.
        } else if (ev->status & FLY_LIST_ACTIVE) {
            ret = fly_insert_queue(ev->core->fly_active_queue, ev) == -1?-2:0;
            switch (ret) {
                case 0:
...
int fly_event_add_to_epoll(fly_core *core)
{
	...
	for (int i = 0; i < fly_queue_length(core->fly_reg_queue); i++) {
        ...
        //use this queue to save the event added into epoll mode.
        ...
        if (event->flags & (FLY_EVENT_READ | FLY_EVENT_WRITE)) {
            ...
            */
        	...
        	if (epoll_ctl(core->ep_info->epoll_fd, op, event->fd, &epev) == -1) {
                ...
                if (fly_delete_queue(core->fly_io_queue, event) == -1) {
...
/*************************************************************************
                       ...
                       please notice!!!
                       below is test code for fly_event.

**************************************************************************/

/*
////////////////Below is test code for pipi's fd.//////////////////
...
//end of test code for pipi's fd.
*/




//test code for time event.
void time_cb() 
{
    printf("time_cb: is here.\n");
}
...
void main()
{
    ...
    fly_event event = malloc(sizeof(struct fly_eventt));
    if (event == NULL) {
        printf(malloc error.\n);
        return;
    }

    if (fly_event_set(-1, time_cb, &event, FLY_EVENT_PERSIST, NULL, core) == -1) {
        printf("fly_event_set error.\n");
        return;
    }

    fly_event_add(&event, &tv);
}
//end of test code for time event.
